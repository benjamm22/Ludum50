...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void HandleSideToSide()
    {
        ...
        if (key != KeyCode.None && timeElapsed < lerpDuration)
        {
            ...
            //t = t * t * (3f - 2f * t);
            valueToLerp = Mathf.Lerp(startValue, endWithDirection, t);
            playerTransform.position = new Vector3(valueToLerp, playerTransform.position.y, playerTransform.position.z);
            timeElapsed += Time.deltaTime;
            AddSnapbackTime();
        }
        else if (key != KeyCode.None)
        {
            ...
            playerTransform.position = new Vector3(endWithDirection, playerTransform.position.y, transform.position.z);
            key = KeyCode.None;
        }
        else
        {
